#!/bin/bash
# Session Resolver接続テストスクリプト
# 導通確認最優先・セキュリティ度外視版

set -e

echo "=== DCV カスタムSession Resolver 接続テスト ==="
echo "実行日時: $(date)"
echo
...
echo
echo "=== Test 2: Agent2直接接続確認 ==="
log_info "Agent2への直接接続状態を確認"

echo "2-1. Agent2 HTTP接続テスト"
if curl -k -s -I https://10.213.66.188:60000/ | grep -q "HTTP/1.1 200 OK"; then
    log_info "✅ Agent2直接接続: OK"
else
    log_error "❌ Agent2直接接続: FAILED"
...
# SSH経由でセッション一覧を取得（実際の環境に合わせて調整）
# ssh -i "tom.pem" ubuntu@10.213.66.188 -p 60001 "sudo dcv list-sessions"
...
if curl -k -s -I https://10.213.66.188:8443/ | head -1; then
...
GATEWAY_RESPONSE=$(curl -k -s -I https://10.213.66.188:8443/?sessionId=console | head -1)
...
echo "直接接続URL (Agent2):"
echo "  https://10.213.66.188:60000/?sessionId=console"
echo
...
echo "2. DCV Gateway設定確認: dcv-connection-gateway-custom.conf"
